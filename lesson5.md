...
## Визначення обмеження DEFAULT

Обмеження DEFAULT використовується для надання стовпцям таблиці значень за замовчуванням. Коли при вставці запису в таблицю не вказується значення для певного стовпця, замість NULL буде вставлено значення за замовчуванням, якщо воно визначене для цього стовпця.

## Випадки застосування DEFAULT обмеження

Розглянемо два основні випадки:
1. Створення нової таблиці з обмеженням DEFAULT
2. Додавання обмеження DEFAULT до існуючої таблиці

## Випадок 1: Створення нової таблиці з обмеженням DEFAULT

Для створення таблиці з обмеженням DEFAULT використовуємо такий синтаксис:
...
У цьому прикладі:
- Створюється таблиця `test_default` з чотирма стовпцями
- Для стовпця `id` встановлюється значення за замовчуванням 5
- Для стовпця `first_name` встановлюється значення за замовчуванням 'Rohit'
- Для стовпців `last_name` та `age` значення за замовчуванням не встановлені

### Приклади вставки даних

1. Вставка з явним вказанням всіх значень:
```sql
INSERT INTO test_default VALUES (1, 'Nitin', 'Gen', 23);
```
У цьому випадку використовуються всі вказані значення, і значення за замовчуванням не застосовуються.

2. Вставка без вказання деяких стовпців:
```sql
INSERT INTO test_default (last_name, age) VALUES ('Singh', 34);
```
У цьому випадку для стовпців `id` та `first_name` будуть використані значення за замовчуванням - 5 та 'Rohit'.

3. Вставка з вказанням лише одного стовпця:
```sql
INSERT INTO test_default (last_name) VALUES ('Grover');
```
Результат такої вставки:
- `id` = 5 (значення за замовчуванням)
- `first_name` = 'Rohit' (значення за замовчуванням)
- `last_name` = 'Grover' (вказане значення)
- `age` = NULL (значення за замовчуванням не встановлено, тому використовується NULL)

## Випадок 2: Додавання обмеження DEFAULT до існуючої таблиці

Щоб додати обмеження DEFAULT до вже існуючої таблиці, використовуємо команду ALTER TABLE:
...
Цей запит додає значення за замовчуванням 25 для стовпця `age`.

### Перевірка роботи обмеження

Вставимо запис, вказавши лише значення для стовпця `last_name`:
```sql
INSERT INTO test_default (last_name) VALUES ('Gen');
```

Результат:
- `id` = 5 (значення за замовчуванням)
- `first_name` = 'Rohit' (значення за замовчуванням)
- `last_name` = 'Gen' (вказане значення)
- `age` = 25 (нове значення за замовчуванням)

Тепер, коли ми не вказуємо значення для стовпця `age`, замість NULL автоматично вставляється значення 25.

## Висновок

Обмеження DEFAULT - корисний інструмент для забезпечення стандартних значень у стовпцях таблиці, коли конкретні значення не вказані при вставці даних. Це допомагає уникнути NULL значень та забезпечує більшу цілісність даних. Обмеження DEFAULT можна встановити як при створенні нової таблиці, так і додати до вже існуючої таблиці за допомогою ALTER TABLE.


# Первинні та зовнішні ключі в SQL

## Визначення первинного ключа (Primary Key)

Первинний ключ у таблиці – це стовпець або група атрибутів, які можна використовувати для розрізнення між будь-якими двома записами в певній таблиці. Первинний ключ має відповідати двом основним вимогам:
- Не може містити NULL-значення
- Не може містити дублікати (тобто всі значення мають бути унікальними)

## Особливості первинного ключа

Розглянемо приклад таблиці `table1` з двома стовпцями:
- `id` (первинний ключ)
- `name`
...
У цій таблиці стовпець `id` є первинним ключем. Він не містить NULL-значень і не має дублікатів.

## Визначення зовнішнього ключа (Foreign Key)

Коли ми працюємо з базами даних, ми зберігаємо дані у вигляді таблиць. Ці таблиці можуть бути пов'язані між собою через відношення первинного та зовнішнього ключів.

## Особливості зовнішнього ключа

Зовнішній ключ:
- Може містити дублікати
- Може містити NULL-значення
- Значення в стовпці зовнішнього ключа (крім NULL) мають бути підмножиною значень у стовпці первинного ключа відповідної таблиці

## Приклад відношення первинного та зовнішнього ключів

Розглянемо другу таблицю `table2` з двома стовпцями:
- `id` (зовнішній ключ)
- `course`
...
У цьому прикладі стовпець `id` у `table2` є зовнішнім ключем, який посилається на первинний ключ `id` у таблиці `table1`.

## Забезпечення референційної цілісності

Важливо зазначити, що неможливо додати запис до `table2` зі значенням `id`, яке не існує в `table1`. Наприклад, якщо спробувати додати запис з `id=6`, це буде неможливо, оскільки такого значення немає в стовпці первинного ключа `table1`.

Це називається підтримкою референційної цілісності, і вона забезпечується через обмеження зовнішнього ключа.

## Ключові відмінності між первинним та зовнішнім ключами

1. **Первинний ключ**:
   - Не може містити NULL-значення
   - Не може містити дублікати
   - Використовується для унікальної ідентифікації записів

2. **Зовнішній ключ**:
   - Може містити NULL-значення
   - Може містити дублікати
   - Значення мають бути підмножиною відповідного первинного ключа
   - Використовується для встановлення зв'язків між таблицями


# Обмеження PRIMARY KEY в SQL

## Визначення обмеження PRIMARY KEY

Обмеження PRIMARY KEY (первинний ключ) визначає унікальний ідентифікатор для кожного запису в таблиці. У попередніх лекціях ми вже обговорювали, що первинний ключ:
- Не може містити NULL-значення
- Не може містити дублікати (значення мають бути унікальними)
- У таблиці може бути лише один первинний ключ

## Випадки застосування PRIMARY KEY обмеження

Розглянемо два основні випадки:
1. Створення нової таблиці з обмеженням PRIMARY KEY
2. Додавання обмеження PRIMARY KEY до існуючої таблиці

## Випадок 1: Створення нової таблиці з обмеженням PRIMARY KEY

Для створення таблиці з первинним ключем використовуємо такий синтаксис:
...
У цьому прикладі:
- Створюється таблиця `test_pk1` з чотирма стовпцями
- Стовпець `eid` визначається як первинний ключ

### Перевірка роботи обмеження

1. Вставимо запис у таблицю:
```sql
INSERT INTO test_pk1 VALUES (1, 'M', 23, 'Mayank');
```
Цей запит виконується успішно.

2. Спробуємо вставити запис із дублікатом значення у стовпці первинного ключа:
```sql
INSERT INTO test_pk1 VALUES (1, 'F', 19, 'Priya');
```
Цей запит завершується помилкою: "Порушення обмеження PRIMARY KEY. Дублікат значення ключа: 1".

3. Спробуємо вставити NULL у стовпець первинного ключа:
```sql
INSERT INTO test_pk1 VALUES (NULL, 'M', 25, 'Raj');
```
Цей запит також завершується помилкою: "Не можна вставити NULL-значення у стовпець 'eid', оскільки стовпець не допускає NULL-значень".

## Випадок 2: Додавання PRIMARY KEY до існуючої таблиці

Для додавання обмеження PRIMARY KEY до існуючої таблиці використовуємо команду ALTER TABLE:
...
1. У таблиці може бути тільки один первинний ключ. Якщо спробувати додати ще один PRIMARY KEY, виникне помилка:
```sql
ALTER TABLE test_pk1
ADD PRIMARY KEY (age);
```
Результат: "Таблиця вже має первинний ключ".

2. Стовпці, на які встановлюється обмеження PRIMARY KEY, повинні бути NOT NULL:
```sql
...
3. Первинний ключ може складатися з декількох стовпців (складений первинний ключ):
```sql
ALTER TABLE test_pk2
ADD PRIMARY KEY (sid, age);
```
У цьому випадку комбінація значень стовпців `sid` та `age` має бути унікальною для кожного запису.

## Видалення таблиці та обмеження

Якщо потрібно змінити первинний ключ, можна видалити таблицю і створити її заново:

```sql
DROP TABLE test_pk2;
```

Або можна спершу видалити існуюче обмеження PRIMARY KEY, а потім додати нове.

## Висновок

Обмеження PRIMARY KEY є важливим інструментом для забезпечення цілісності даних у таблицях SQL. Воно гарантує унікальність кожного запису та запобігає вставці NULL-значень у ключові стовпці. PRIMARY KEY можна визначити як для одного стовпця, так і для комбінації стовпців.



# Обмеження FOREIGN KEY в SQL

## Визначення обмеження FOREIGN KEY

Обмеження FOREIGN KEY (зовнішній ключ) встановлює зв'язок між даними у двох таблицях. Зовнішній ключ у одній таблиці посилається на первинний ключ в іншій таблиці, забезпечуючи референційну цілісність даних.

## Випадки застосування FOREIGN KEY обмеження

Розглянемо два основні випадки:
1. Створення нової таблиці з обмеженням FOREIGN KEY
2. Додавання обмеження FOREIGN KEY до існуючої таблиці

## Випадок 1: Створення нової таблиці з обмеженням FOREIGN KEY

### Крок 1: Спочатку створюємо таблицю з первинним ключем
...
Вставимо тестові дані у таблицю з первинним ключем:
...
### Крок 2: Створюємо таблицю із зовнішнім ключем
...
У цьому прикладі:
- Створюється таблиця `test_foreign_key` з двома стовпцями
- Стовпець `id` визначається як зовнішній ключ, який посилається на стовпець `id` у таблиці `test_primary_key`

### Перевірка роботи обмеження

1. Вставимо допустимий запис:
```sql
INSERT INTO test_foreign_key VALUES (1, 'A');
```
Запит виконується успішно, оскільки значення 1 існує в таблиці `test_primary_key`.

2. Можна вставити NULL у стовпець зовнішнього ключа:
```sql
INSERT INTO test_foreign_key VALUES (NULL, 'B');
```
Запит виконується успішно, оскільки зовнішній ключ може містити NULL-значення.

3. Спробуємо вставити значення, якого немає в таблиці з первинним ключем:
```sql
INSERT INTO test_foreign_key VALUES (5, 'C');
```
Цей запит завершується помилкою: "Інструкція INSERT конфліктує з обмеженням FOREIGN KEY. Конфлікт виник у базі даних у таблиці 'test_primary_key', стовпець 'id'".

## Випадок 2: Додавання обмеження FOREIGN KEY до існуючої таблиці

Для додавання обмеження FOREIGN KEY до вже існуючої таблиці, виконуємо наступні кроки:

### Крок 1: Створюємо таблицю без обмеження FOREIGN KEY
...
### Крок 2: Додаємо обмеження FOREIGN KEY
...
Цей запит додає обмеження зовнішнього ключа до стовпця `id` у таблиці `test_foreign_key_2`, посилаючись на стовпець `id` у таблиці `test_primary_key`.

## Важливі особливості FOREIGN KEY

1. Зовнішній ключ може містити NULL-значення, на відміну від первинного ключа
2. Зовнішній ключ може містити дублікати значень
3. Значення у стовпці зовнішнього ключа (крім NULL) повинні існувати у відповідному стовпці первинного ключа
4. Зовнішній ключ забезпечує референційну цілісність даних між таблицями

## Висновок

Обмеження FOREIGN KEY є важливим інструментом для встановлення зв'язків між таблицями та забезпечення цілісності даних у реляційних базах даних. Воно гарантує, що значення у стовпці зовнішнього ключа відповідають значенням у стовпці первинного ключа пов'язаної таблиці, запобігаючи вставці недійсних даних.


# Порядок виконання запитів у Microsoft SQL Server

## Вступ до порядку виконання

Порядок, у якому ми пишемо SQL-запит, та порядок, у якому він виконується, відрізняються. Це важливе питання, яке часто зустрічається на співбесідах, незалежно від вашого рівня досвіду.

## Приклад запиту

Для пояснення розглянемо наступний запит, який обчислює середню зарплату за відділами:
...
У цьому запиті ми:
- Вибираємо департамент і середню зарплату
- З таблиці employee_salaries
- Для записів із зарплатою понад 50000
- Групуємо результати за департаментами
- Фільтруємо групи, де середня зарплата перевищує 55000
- Сортуємо за назвою департаменту
- Вибираємо тільки унікальні значення та лише перший запис

## Фактичний порядок виконання

Хоча запит написаний в одному порядку, виконання відбувається в іншому:

1. **FROM та JOIN**: Спочатку виконуються секції FROM та JOIN, які визначають, з яких таблиць буде отримано дані
2. **WHERE**: Потім виконується фільтрація рядків за умовою WHERE
3. **GROUP BY**: Далі відбувається групування даних
4. **HAVING**: Фільтрація груп за умовою HAVING
5. **SELECT**: Вибір стовпців та обчислення виразів
6. **DISTINCT**: Видалення дублікатів
7. **ORDER BY**: Сортування результатів
8. **TOP**: Обмеження кількості повернутих рядків

## Практичний приклад важливості порядку виконання

Щоб зрозуміти важливість порядку виконання, розглянемо помилку, яка може виникнути:

Якщо спробувати використати аліас стовпця, визначеного в SELECT, у виразі HAVING:
...
WHERE salary > 50000
GROUP BY department
HAVING [Average Salary] > 55000  -- Ця частина викликає помилку
ORDER BY department;
```

Отримаємо помилку "Недійсне ім'я стовпця 'Average Salary'".

Це відбувається тому, що HAVING виконується **до** SELECT, тому аліаси стовпців, визначені в SELECT, ще не доступні для використання в HAVING. Правильно буде повторно написати вираз:
...
Розуміння порядку виконання запитів у SQL Server має вирішальне значення для:
- Написання правильних та ефективних запитів
- Уникнення помилок при використанні аліасів
- Оптимізації продуктивності запитів
- Успішного проходження технічних співбесід

Знання порядку виконання операторів (FROM → WHERE → GROUP BY → HAVING → SELECT → DISTINCT → ORDER BY → TOP) є фундаментальним для ефективної роботи з SQL.
